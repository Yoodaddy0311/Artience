...
// 세션 복원 시 --resume 사용
function buildAutoCommand(agent: AgentProfile, cwd: string, sessionId?: string): string {
...
async function createAgentWorktree(agentId: string, cwd: string): Promise<string> {
    const branchName = `agent/${agentId}`;
    const worktreePath = path.join(cwd, `.claude/worktrees/${agentId}`);

    await exec(`git worktree add -b ${branchName} ${worktreePath} HEAD`, { cwd });
...
async function ensureClaudeMd(cwd: string, agent: AgentProfile): Promise<void> {
    ...
    await fs.promises.writeFile(claudeMdPath, `
...
function buildTeamMeetingCommand(
    ...
    const agents = members.reduce((acc, m) => {
        acc[m.name.toLowerCase()] = {
            description: `${m.name} - ${m.role}`,
            prompt: buildSystemPrompt(m.name),
            tools: AGENT_ALLOWED_TOOLS[m.name.toLowerCase()] || ['Read', 'Glob', 'Grep'],
...
interface ParsedChunk {
    type: 'user_prompt' | 'assistant_text' | 'tool_use' | 'tool_result'
        | 'thinking' | 'permission_request' | 'error' | 'raw';
    content: string;
    toolName?: string;
    toolInput?: string;
}
...
{chunk.type === 'tool_use' && (
    <details className="bg-[#313244] rounded p-2 text-xs">
        ...
        <pre className="text-[#a6adc8] mt-1 whitespace-pre-wrap">{chunk.content}</pre>
    </details>
)}
