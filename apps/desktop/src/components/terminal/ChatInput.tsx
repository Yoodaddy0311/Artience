...
const NATIVE_COMMANDS: SlashCommand[] = [
    { id: '/compact', label: '/compact', description: '대화 컨텍스트 압축', source: 'native' },
    { id: '/clear', label: '/clear', description: '대화 내용 초기화', source: 'native' },
    { id: '/help', label: '/help', description: '도움말 표시', source: 'native' },
    { id: '/bug', label: '/bug', description: '버그 리포트', source: 'native' },
    { id: '/memory', label: '/memory', description: '메모리 관리', source: 'native' },
    { id: '/review', label: '/review', description: '코드 리뷰', source: 'native' },
    { id: '/init', label: '/init', description: '프로젝트 초기화', source: 'native' },
    { id: '/config', label: '/config', description: '설정 관리', source: 'native' },
    { id: '/mcp', label: '/mcp', description: 'MCP 서버 관리', source: 'native' },
    { id: '/cost', label: '/cost', description: '비용 확인', source: 'native' },
    { id: '/vim', label: '/vim', description: 'Vim 모드 토글', source: 'native' },
    { id: '/doctor', label: '/doctor', description: '진단 실행', source: 'native' },
    { id: '/status', label: '/status', description: '상태 확인', source: 'native' },
    ...
    return (
        ...
        >
            {commands.map((cmd, i) => (
                ...
                >
                    <span className="text-sm font-bold text-black">{cmd.label}</span>
                    <span className="text-xs text-gray-500 truncate flex-1">{cmd.description}</span>
                    {cmd.source === 'skill' && (
                        <span className="text-[9px] font-bold text-purple-500 border border-purple-300 rounded px-1">skill</span>
                    )}
                </button>
            ))}
        </div>
        ...
        <div className="flex flex-wrap gap-1.5 px-2 pt-2">
            {attachments.map((att) => (
                ...
                >
                    ...
                    <span className="text-[10px] font-bold text-gray-700 max-w-[100px] truncate">{att.name}</span>
                    <button
...
export const ChatInput: React.FC<ChatInputProps> = ({ tabId, agentSprite, onSubmit, disabled }) => {
    ...
    const allCommands = useMemo(() => [...NATIVE_COMMANDS, ...projectSkills], [projectSkills]);
    ...
    const processFileBlob = useCallback(async (file: File): Promise<Attachment | null> => {
        const ext = getFileExt(file.name);
        if (!isSupportedFile(file.name) && !file.type.startsWith('image/')) return null;
    ...
    const saveAttachmentToTemp = useCallback(async (att: Attachment): Promise<string | null> => {
    ...
    const handleSubmit = useCallback(async () => {
        ...
        if (message) {
            onSubmit(message);
            useTerminalStore.getState().addInputHistory(tabId, trimmed || paths.join(' '));
        }
    ...
    }, [text, attachments, onSubmit, tabId, saveAttachmentToTemp]);
    ...
    const handleKeyDown = useCallback((e: React.KeyboardEvent<HTMLTextAreaElement>) => {
        ...
        if (e.key === 'ArrowUp' && !e.shiftKey && !slashOpen) {
            ...
            if ((text === '' || isBrowsing) && cursorAtStart && inputHistory.length > 0) {
                e.preventDefault();
                if (historyIndex < 0) setSavedText(text);
                const newIdx = historyIndex < 0
        ...
        if (e.key === 'ArrowDown' && !e.shiftKey && !slashOpen && historyIndex >= 0) {
            e.preventDefault();
    ...
    }, [slashOpen, filteredCommands, slashIndex, handleSlashSelect, text, historyIndex, inputHistory, savedText, handleSubmit]);
            ...
            {isDragOver && (
                <div className="absolute inset-0 bg-[#E8DAFF]/80 border-2 border-dashed border-[#A78BFA] rounded-lg flex items-center justify-center z-40 pointer-events-none">
                    <div className="text-center">
                        ...
                        <p className="text-sm font-bold text-black mt-1">파일을 놓으세요</p>
                    </div>
                </div>
            )}
        </div>
