...
export const RunPanel: React.FC<{ onClose: () => void }> = ({ onClose }) => {
    ...
    const [expandedTypes, setExpandedTypes] = useState<Record<string, boolean>>(
        { image: true, document: true, archive: true, other: true },
    );
    ...
    const runRecipe = async (recipeId: string) => {
        ...
        const agent =
            idle[Math.floor(Math.random() * idle.length)] || agents[0];
        const jobApi = window.dogbaApi?.job;
        if (!jobApi) return;
        try {
            const result = await jobApi.run(
                agent.name,
    ...
    const stopJob = (jobId: string) => {
        setJobs((prev) =>
            prev.map((j) =>
                j.id === jobId
                    ? { ...j, state: 'CANCELED' as const, endedAt: Date.now() }
                    : j,
            ),
        );
    };
    ...
    return (
        ...
        >
            ...
            <div
                className="flex border-b-2 border-black bg-white"
                role="tablist"
            ...
            >
                {(
                    ...
                    <button
                        ...
                        aria-controls={`run-tabpanel-${key}`}
                        id={`run-tab-${key}`}
                        onClick={() => setTab(key)}
                        className={`flex-1 py-3 text-xs font-black transition-all border-r-2 border-black last:border-r-0 focus-visible:ring-2 focus-visible:ring-black focus-visible:ring-offset-2 ${tab === key ? 'text-black bg-[#E8DAFF]' : 'text-gray-500 hover:bg-gray-50'}`}
                    >
                        {label}
                    </button>
                ))}
            ...
            <div className="flex-1 overflow-y-auto">
                ...
                {tab === 'agents' && (
                    <div className="p-3 space-y-1.5">
                        ...
                        <div className="flex gap-2 flex-wrap mb-3 p-2 bg-gray-50 rounded-lg border-2 border-black">
                            {Object.entries(stateCounts).map(
                                ([state, count]) => (
                                    <span
                                        key={state}
                                        className={`text-xs font-bold px-2 py-1 rounded-md border border-black ${STATE_COLORS[state as AgentState]} ${state === 'THINKING' ? 'text-black' : 'text-white'} shadow-[1px_1px_0_0_#000]`}
                                    >
                        ...
                        {agents.map((agent) => (
                            <div
                                key={agent.id}
                                className="flex flex-col gap-2 p-2.5 rounded-lg hover:bg-gray-50 transition-colors border-2 border-transparent hover:border-black cursor-pointer"
                                onClick={() =>
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    <img
                                        ...
                                        alt={agent.name}
                                        className="w-10 h-10 rounded-lg bg-gray-100 object-contain border-2 border-black"
                                    />
                                            ...
                                            {(() => {
                                                ...
                                                return agentJobs
                                                    ...
                                                    .map((j, idx) => (
                                                        <div
                                                            key={idx}
                                                            className={`py-0.5 ${j.state === 'ERROR' ? 'text-red-400' : 'text-green-300'}`}
                                                        >
                ...
                {tab === 'jobs' && (
                    <div className="p-4 space-y-4">
                        {/* Recipe Buttons */}
                        <div>
                            <h3 className="text-xs font-black text-black uppercase mb-2">
                                레시피 (클릭하여 실행)
                            </h3>
                            <div className="grid grid-cols-2 gap-2">
                                {recipes.map((r) => (
                                    <button
                                        ...
                                        className="p-3 bg-white rounded-lg border-2 border-black text-left hover:bg-[#9DE5DC] hover:shadow-[3px_3px_0_0_#000] transition-all shadow-[2px_2px_0_0_#000] group"
                                    >
                                        <span className="text-sm font-black text-black group-hover:text-black block">
                                            {r.name}
                                        </span>
                        ...
                        <div>
                            ...
                            {jobs.length === 0 ? (
                                <p className="text-sm text-gray-400 text-center py-6 border-2 border-dashed border-gray-300 rounded-lg">
                                    실행된 작업이 없습니다
                                </p>
                            ) : (
                                <div className="space-y-2">
                                    {jobs
                                        ...
                                        .map((job) => (
                                            <div
                                                key={job.id}
                                                className="p-3 bg-white rounded-lg border-2 border-black shadow-[2px_2px_0_0_#000]"
                                            >
                                                <div className="flex justify-between items-center">
                                                    ...
                                                    <span
                                                        className={`text-xs font-black px-2 py-0.5 rounded-md border-2 border-black ${job.state === 'SUCCESS' ? 'bg-emerald-200 text-emerald-800' : job.state === 'RUNNING' ? 'bg-green-200 text-green-800 animate-pulse' : job.state === 'ERROR' ? 'bg-red-200 text-red-800' : job.state === 'QUEUED' ? 'bg-yellow-200 text-yellow-800' : 'bg-gray-200 text-gray-600'}`}
                                                    >
                                                        {job.state}
                                                    </span>
                                                </div>
                            ...
                            <div className="space-y-4">
                                {Object.entries(
                                    ...
                                    <div key={type}>
                                        ...
                                        >
                                            <span
                                                className={`transform transition-transform ${expandedTypes[type] ? 'rotate-90' : ''}`}
                                            >
                                                ▶
                                            </span>
                                        ...
                                        {expandedTypes[type] && (
                                            <div className="pl-4 ml-2 border-l-2 border-dashed border-gray-300 space-y-2">
                                                {list.map((art, i) => (
                                                    <div
                                                        key={i}
                                                        className="flex items-center gap-3 p-3 bg-white rounded-lg border-2 border-black shadow-[2px_2px_0_0_#000] hover:shadow-[3px_3px_0_0_#000] transition-all"
                                                    >
                    ...
                    <div className="p-4 space-y-5">
                        <div className="flex items-center justify-between mb-3">
                            ...
                            {settingsSyncStatus === 'saved' && (
                                <span className="text-xs font-bold text-emerald-600 px-2 py-0.5 bg-emerald-50 border border-emerald-300 rounded-md">
                                    Saved
                                </span>
                            )}
                            {settingsSyncStatus === 'error' && (
                                <span className="text-xs font-bold text-red-600 px-2 py-0.5 bg-red-50 border border-red-300 rounded-md">
                                    저장 실패
                                </span>
                            )}
                        </div>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            />
                            <label
                                htmlFor="setting-auto-scroll"
                                className="text-xs font-black text-black cursor-pointer"
                            >
                                로그 자동 스크롤
                            </label>
                        </div>
