...
const LEVEL_COLORS: Record<
    number,
    { bg: string; text: string; segment: string }
> = {
...
export function LevelProgress({
    ...
    const progressPercent =
        totalForLevel > 0 ? (levelProgress / totalForLevel) * 100 : 0;
    ...
    const activeSegments = Math.round(
        (progressPercent / 100) * classes.segments,
    );

    return (
        <div
            className={cn('flex flex-col group', classes.container, className)}
        >
            <style>{`
      ...
      `}</style>
            ...
            <div className="flex items-center justify-between pointer-events-auto">
                <div className="flex items-center gap-2">
                    ...
                    <span
                        ...
                        style={{ WebkitTextStroke: '0.2px #18181b' }}
                    >
                        {title}
                    </span>
                </div>
                ...
                {showDetails && (
                    <span
                        className={cn(
                            'font-bold tabular-nums text-gray-500',
                            classes.details,
                        )}
                    >
            ...
            <div className="space-y-1.5 pointer-events-auto">
                ...
                >
                    ...
                    {activeSegments > 0 && (
                        <div className="level-progress-shimmer" />
                    )}
                </div>
                ...
                {showDetails && (
                    <div className="flex justify-between mt-2">
                        ...
                        >
                            EXP {levelProgress.toLocaleString()} /{' '}
                            {totalForLevel.toLocaleString()}
                        </span>
                        <span
                            className={cn(
