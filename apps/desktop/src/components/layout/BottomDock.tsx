...
import { useTerminalStore, type AgentSettings } from '../../store/useTerminalStore';
...
// Dokba(raccoon)는 기본 CTO — DEFAULT_AGENTS에 없으므로 별도 정의
...
export const BottomDock: React.FC = () => {
    ...
    const [settingsPopup, setSettingsPopup] = useState<{ agentId: string } | null>(null);
    const [settingsDraft, setSettingsDraft] = useState<AgentSettings>({});

    // --- 캐릭터 클릭: 세션 생성 또는 활성화 ---
    const handleCharacterClick = useCallback(async (agentId: string) => {
        ...
        const result = await api.create(80, 24, { cwd, label: agent.name, autoCommand: 'claude', agentSettings: agentSettings[agentId] });
    ...
    }, [tabs, characterDirMap, agentSettings, setActiveTab, addTab, setCharacterDir]);
    ...
    const handleChangeDir = useCallback(async () => {
        ...
        const result = await api.create(80, 24, { cwd: selected, label: agent.name, autoCommand: 'claude', agentSettings: agentSettings[agentId] });
        if (result?.id) {
            addTab({
                id: result.id,
                agentId,
    ...
    const handleSaveSettings = useCallback(async () => {
        ...
        const result = await api.create(80, 24, { cwd, label: agent.name, autoCommand: 'claude', agentSettings: settingsDraft });
        if (result?.id) {
            addTab({
                id: result.id,
                agentId,
    ...
    }, [settingsPopup, settingsDraft, tabs, characterDirMap, removeTab, addTab, setAgentSettings]);
    ...
    return (
        <>
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-30 flex items-end gap-2">
                ...
                {(() => {
                    ...
                    return (
                        <button
                            ...
                            className={`relative flex flex-col items-center px-4 pt-2 pb-1.5 rounded-2xl transition-all duration-200 bg-white/95 backdrop-blur-sm shadow-[4px_4px_0_0_#000] border-3 border-yellow-500 ${
                                isActive
                        ...
                        >
                            ...
                            <div className="relative">
                                ...
                                {restoring && !hasTab && characterDirMap[CTO_ID] && (
                            ...
                            {dokbaActivityLabel && (
                                <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full mt-0.5 ${
                                    dokbaActivity === 'working' || dokbaActivity === 'thinking'
                ...
                <div className="relative">
                    <button
                        onClick={(e) => { e.stopPropagation(); setAddPopup(prev => !prev); }}
                        title="에이전트 추가"
                            ...
                            {availableAgents.length === 0 ? (
                                ...
                                availableAgents.map(a => (
                                    ...
                                    >
                                        ...
                                        <span className="text-gray-400 text-[10px]">{a.role}</span>
                                    </button>
                                ))
                            )}
                        </div>
            ...
            {settingsPopup && (() => {
                ...
                return (
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30" onClick={() => setSettingsPopup(null)}>
                        <div
                        ...
                        >
                            ...
                            <div className="px-3 py-2 border-b border-gray-200 flex items-center justify-between">
                                <span className="text-xs font-bold">{agent?.name || settingsPopup.agentId} 설정</span>
                                <button onClick={() => setSettingsPopup(null)} className="text-gray-400 hover:text-black text-xs font-bold">x</button>
                            </div>
                            ...
                            <div className="px-3 py-2 flex flex-col gap-2">
                                ...
                                <div className="flex flex-col gap-0.5">
                                    <label className="text-[10px] font-bold text-gray-500">모델</label>
                                    <select
                                        value={settingsDraft.model || ''}
                                        onChange={(e) => setSettingsDraft(d => ({ ...d, model: (e.target.value || undefined) as AgentSettings['model'] }))}
                                        className="w-full px-2 py-1.5 text-xs border-2 border-black rounded font-bold bg-white shadow-[1px_1px_0_0_#000]"
                                    >
                                        <option value="">기본 (sonnet)</option>
                                ...
                                {/* 권한 모드 */}
                                <div className="flex flex-col gap-0.5">
                                    <label className="text-[10px] font-bold text-gray-500">권한 모드</label>
                                    <select
                                        value={settingsDraft.permissionMode || 'default'}
                                        onChange={(e) => setSettingsDraft(d => ({ ...d, permissionMode: e.target.value as AgentSettings['permissionMode'] }))}
                                        className="w-full px-2 py-1.5 text-xs border-2 border-black rounded font-bold bg-white shadow-[1px_1px_0_0_#000]"
                                    >
                                        <option value="default">Default (확인 필요)</option>
                                        <option value="plan">Plan (읽기 전용)</option>
                                        <option value="bypassPermissions">Bypass (자동 승인)</option>
                                    </select>
                                </div>

                                {/* 최대 턴 */}
                                <div className="flex flex-col gap-0.5">
                                    <label className="text-[10px] font-bold text-gray-500">최대 턴</label>
                                    <input
                                        type="number"
                                        min={1}
                                        max={100}
                                        ...
                                        onChange={(e) => {
                                            ...
                                            setSettingsDraft(d => ({ ...d, maxTurns: isNaN(val) ? undefined : val }));
                                        }}
