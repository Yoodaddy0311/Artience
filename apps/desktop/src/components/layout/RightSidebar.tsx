...
import { MessageCircle, Paperclip, Activity, Zap } from 'lucide-react';
...
export const RightSidebar: React.FC<{ agent: Teammate; onClose: () => void }> = ({ agent, onClose }) => {
    ...
    useEffect(() => {
        ...
        if (chatApi?.getSkills) {
            chatApi.getSkills(agent.name).then((result) => {
    ...
    const send = async (e: React.FormEvent) => {
        ...
        if (chatApi) {
            ...
            try {
                setStreamingText('');
                const result = await chatApi.sendStream(agent.name, currentMsg, activeSkillId ?? undefined);
                if (!result.success) {
                    setIsTyping(false);
                    setChats((prev) => [
                        ...prev,
    ...
    return (
        <div className="w-full max-w-md h-full bg-white border-l-2 border-black flex flex-col z-20 transition-all absolute right-0 top-0">
            ...
            {showSkillPicker && skills.length > 0 && (
                <div className="px-4 py-2 bg-cream-50 border-t-2 border-black">
                    <div className="flex flex-wrap gap-1.5">
                        <button
                            onClick={() => { setActiveSkillId(null); setShowSkillPicker(false); }}
                            className={`px-3 py-1.5 text-xs font-bold rounded-lg border-2 border-black transition-all ${
                                !activeSkillId
                                    ? 'bg-[#FFD100] shadow-[2px_2px_0_0_#000]'
                        ...
                        {skills.map((skill) => (
                            <button
                                key={skill.id}
                                onClick={() => { setActiveSkillId(skill.id); setShowSkillPicker(false); }}
                                title={skill.description}
                                className={`px-3 py-1.5 text-xs font-bold rounded-lg border-2 border-black transition-all ${
                                    activeSkillId === skill.id
                                        ? 'bg-[#A78BFA] text-white shadow-[2px_2px_0_0_#000]'
                                        : 'bg-white hover:bg-gray-100'
                                }`}
                            >
                                {skill.label}
            ...
            <form onSubmit={send} className="p-4 bg-white border-t-2 border-black flex flex-col gap-2">
                ...
                {activeSkill && (
                    <div className="flex items-center gap-2 px-2 py-1">
                        ...
                        <span className="text-xs font-bold text-purple-600">{activeSkill.label}</span>
                        <button
                            type="button"
                ...
                <div className="flex gap-2">
                    ...
                    {skills.length > 0 && (
                        <button
                            ...
                            className={`w-12 h-12 rounded-lg text-black flex items-center justify-center border-2 border-black shadow-[2px_2px_0_0_#000] hover:-translate-y-0.5 hover:shadow-[4px_4px_0_0_#000] active:translate-y-0.5 active:shadow-none transition-all shrink-0 ${
                                activeSkillId ? 'bg-[#A78BFA] text-white' : 'bg-white'
                            }`}
                            title="Skill 선택"
                        >
                    ...
                    <input type="text" placeholder={activeSkill ? `[${activeSkill.label}] ${agent.name}에게 요청...` : `${agent.name}에게 요청할 작업 입력...`} className="flex-1 px-4 py-3 rounded-lg border-2 border-black focus:outline-none focus:shadow-[3px_3px_0_0_#000] transition-shadow text-sm font-bold" value={msg} onChange={(e) => setMsg(e.target.value)} />
