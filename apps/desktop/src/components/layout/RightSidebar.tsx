...
export const RightSidebar: React.FC<{ agent: Teammate, onClose: () => void }> = ({ agent, onClose }) => {
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            if (socketRef.current?.readyState === WebSocket.OPEN) {
                socketRef.current.send(JSON.stringify({ type: 'CHAT_COMMAND', text: `새 파싱 문서가 도착했습니다 (${file.name}). 분석을 시작하세요.`, target_agent: 'Sera' }));
            }
        } catch (error) {
    ...
    useEffect(() => {
        ...
        ws.onmessage = (event) => {
            ...
            if (data.type === 'TASK_ASSIGNED' && (data.agent === agent.name || data.agent.toLowerCase() === agent.name.toLowerCase())) {
                setIsTyping(false);
                setChats(prev => [...prev, {
                    sender: data.agent,
                    text: data.taskContent === '작업 처리 중...' ? 'API와 통신하여 작업을 할당했습니다!' : `할당됨: ${data.taskContent}`
                }]);
            }
        };
    ...
    const send = (e: React.FormEvent) => {
        ...
        setChats(prev => [...prev, { sender: 'Me', text: msg }]);
        setIsTyping(true);

        if (socketRef.current?.readyState === WebSocket.OPEN) {
            socketRef.current.send(JSON.stringify({ type: 'CHAT_COMMAND', text: msg, target_agent: agent.name }));
        }
                ...
                <div className="flex gap-4 items-center">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-1.5 mt-1">
                            <span className={`w-2.5 h-2.5 rounded-full border border-black ${agent.status === 'working' ? 'bg-green-400' : 'bg-amber-300'}`} />
            ...
            <div className="p-4 grid grid-cols-2 gap-3 border-b-2 border-black bg-cream-50">
                <div className="bg-white p-4 rounded-lg border-2 border-black shadow-[4px_4px_0_0_#000]">
                    <div className="flex items-center gap-2 mb-1">
                        ...
                        <p className="text-xs font-bold text-black">오늘 대화</p>
                    </div>
                    <p className="text-2xl font-black text-black">12<span className="text-sm font-bold text-gray-500 ml-1">건</span></p>
                </div>
                <div className="bg-white p-4 rounded-lg border-2 border-black shadow-[4px_4px_0_0_#000]">
                    <div className="flex items-center gap-2 mb-1">
                        <span>⚡</span>
                    ...
                    <p className="text-2xl font-black text-black">98<span className="text-sm font-bold text-gray-500 ml-1">%</span></p>
                </div>
            </div>
            ...
            <div ref={scrollRef} className="flex-1 p-5 overflow-y-auto flex flex-col gap-4 bg-white scroll-smooth">
                {chats.map((c, i) => (
                    <div key={i} className={`flex flex-col max-w-[85%] ${c.sender === 'Me' ? 'self-end items-end' : 'self-start items-start'}`}>
                        ...
                        <div className={`p-3 rounded-lg text-sm leading-relaxed border-2 border-black ${c.sender === 'Me'
