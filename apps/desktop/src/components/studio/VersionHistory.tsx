...
function computeDiff(
    ...
    const numericFields: Array<{
        ...
        },
        {
            key: 'collisionTiles',
            label: '충돌 타일',
            icon: <Blocks className="w-4 h-4" strokeWidth={2.5} />,
        },
        {
            key: 'spawnPoints',
            label: '스폰 포인트',
...
const DiffRow: React.FC<{ item: DiffItem }> = ({ item }) => {
    ...
    return (
        <div
            className={`flex items-center gap-2 p-2.5 rounded-lg border-2 border-black ${style.bg} shadow-[1px_1px_0_0_#000]`}
        >
            <span className="text-base flex-shrink-0 flex items-center">
                {item.icon}
            </span>
            <span className="text-xs font-black text-black flex-shrink-0 w-20">
                {item.label}
            </span>
            <div className="flex-1 flex items-center justify-center gap-1.5">
                <span
                    className={`text-xs font-bold ${item.type === 'unchanged' ? 'text-gray-400' : 'text-gray-600'}`}
                >
                    {item.oldVal}
                </span>
            ...
            <span
                className={`text-[10px] font-black px-2 py-0.5 rounded border-2 border-black ${style.bg} ${style.text} flex-shrink-0`}
            >
                {deltaLabel()}
            </span>
        </div>
...
> = {
    ...
    modified: {
        bg: 'bg-amber-50',
        text: 'text-amber-700',
        icon: <PenLine className="w-3 h-3" strokeWidth={2.5} />,
        label: '수정',
    },
};
...
const JsonDiffRow: React.FC<{ change: JsonDiffChange }> = ({ change }) => {
    ...
    return (
        <div
            className={`flex items-start gap-2 p-2 rounded-lg border-2 border-black ${style.bg} shadow-[1px_1px_0_0_#000]`}
        >
            <span className={`flex-shrink-0 mt-0.5 ${style.text}`}>
                {style.icon}
            </span>
            <div className="flex-1 min-w-0">
                <span className="text-[10px] font-mono font-bold text-black break-all">
                    {change.path}
                </span>
                <div className="flex flex-wrap gap-1 mt-1">
                    ...
                    {change.type === 'modified' && (
                        <>
                            ...
                            <span className="text-[9px] font-black text-black self-center">
                                &rarr;
                            </span>
                            {change.newValue !== undefined && (
                                <span className="text-[9px] font-mono px-1.5 py-0.5 bg-emerald-100 text-emerald-600 rounded border border-emerald-300 break-all">
            ...
            <span
                className={`text-[9px] font-black px-1.5 py-0.5 rounded border border-black ${style.bg} ${style.text} flex-shrink-0`}
            >
                {style.label}
            </span>
        </div>
...
const JsonDiffSection: React.FC<{ oldId: string; newId: string }> = ({
    oldId,
    newId,
}) => {
    ...
    useEffect(() => {
        ...
        const fetchDiff = async () => {
            ...
            try {
                ...
                if (!result.success)
                    throw new Error(result.diff || 'Diff failed');
    ...
    if (loading) {
        return (
            <div className="text-center py-4 text-gray-400 text-xs font-bold animate-pulse">
                JSON diff 로딩 중...
            </div>
        );
    }
    if (error) {
        return (
            <div className="text-center py-4 text-red-400 text-xs font-bold">
                {error}
            </div>
        );
    }
                ...
                <div className="flex gap-2 text-[10px]">
                    <div className="flex-1 p-1.5 bg-white/60 rounded border border-black/20">
                        ...
                        <span className="text-gray-500 ml-1">
                            ({formatDate(olderSnap.createdAt)})
                        </span>
                    </div>
                    <div className="flex-1 p-1.5 bg-white/60 rounded border border-black/20">
                        ...
                        <span className="text-gray-500 ml-1">
                            ({formatDate(newerSnap.createdAt)})
                        </span>
                    </div>
                </div>
            </div>
...
export const VersionHistory: React.FC = () => {
    ...
    const rollback = async (id: string) => {
        ...
        try {
            ...
            setMessage(
                result.success ? '롤백 완료' : result.error || '롤백 실패',
            );
            await fetchHistory();
        } catch {
            setMessage('롤백 실패');
    ...
    return (
        <div className="flex flex-col h-full bg-white">
            ...
            <div className="flex-1 overflow-y-auto p-4">
                {loading ? (
                    <div className="text-center text-gray-400 font-bold animate-pulse py-8">
                        로딩 중...
                    </div>
                ) : snapshots.length === 0 ? (
                    ...
                    <div className="relative">
                        {/* Timeline line */}
                        <div
                            className={`absolute top-0 bottom-0 w-0.5 bg-black ${compareMode ? 'left-10' : 'left-4'}`}
                        />
                        ...
                        <div className="space-y-4">
                            {snapshots.map((snap, i) => {
                                ...
                                if (snap.summary && prevSnap?.summary) {
                                    if (
                                        snap.summary.rooms !==
                                        prevSnap.summary.rooms
                                    )
                                        diffs.push(
                                            `방: ${prevSnap.summary.rooms} → ${snap.summary.rooms}`,
                                        );
                                    if (
                                        snap.summary.agents !==
                                        prevSnap.summary.agents
                                    )
                                        diffs.push(
                                            `Agent: ${prevSnap.summary.agents} → ${snap.summary.agents}`,
                                        );
                                    if (
                                        snap.summary.recipes !==
                                        prevSnap.summary.recipes
                                    )
                                        diffs.push(
                                            `Recipe: ${prevSnap.summary.recipes} → ${snap.summary.recipes}`,
                                        );
                                }
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        {/* Timeline dot */}
                                        <div
                                            className={`w-8 h-8 rounded-full border-2 border-black flex items-center justify-center z-10 flex-shrink-0 ${
                                                isSelected
                                        ...
                                        <div
                                            className={`flex-1 p-3 rounded-lg border-2 border-black bg-white shadow-[2px_2px_0_0_#000] hover:shadow-[3px_3px_0_0_#000] transition-all ${
                                                isSelected
                                                    ? 'ring-2 ring-[#8B5CF6] ring-offset-1'
                                                    : ''
                                            }`}
                                            onClick={
                                                compareMode
                                            ...
                                            <div className="flex items-start justify-between gap-2">
                                                <div className="min-w-0">
                                                        ...
                                                        <div className="flex gap-1 flex-wrap mt-1">
                                                            {diffs.map(
                                                                (d, di) => (
                                                                    <span
                                                                        key={di}
                                                                        className="text-[9px] font-bold px-1.5 py-0.5 bg-blue-100 text-blue-800 border border-blue-300 rounded"
                                                                    >
                                                                        {d}
                                                                    </span>
                                                                ),
                                                    ...
                                                    {diffs.length === 0 &&
                                                        hasSummary &&
                                                        prevSnap?.summary && (
                                                            <span className="text-[9px] font-bold px-1.5 py-0.5 bg-gray-100 text-gray-500 border border-gray-300 rounded">
                                                                변경사항 없음
                                                            </span>
                                                        )}
                                                </div>
            ...
            <ConfirmDialog
                ...
                message={
                    rollbackTarget
                        ? `"${rollbackTarget.label}" 버전으로 롤백하시겠습니까? 현재 상태는 새 스냅샷으로 자동 저장됩니다.`
                        : ''
                }
                confirmLabel="롤백"
                cancelLabel="취소"
