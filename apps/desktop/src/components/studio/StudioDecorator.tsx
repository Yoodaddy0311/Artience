...
interface StudioDecoratorProps {
    onClose?: () => void;
}
    ...
    useEffect(() => {
        try {
            ...
            studioApi
                ...
                .catch((err: any) => {
                    console.warn(
                        '[StudioDecorator] getAssets failed:',
                        err?.message,
                    );
                })
                .finally(() => {
            ...
            console.error(
                '[StudioDecorator] useEffect error:',
                err?.message,
                err?.stack,
            );
            setLoading(false);
    ...
    return (
        <div className="w-full h-full bg-cream-50 flex flex-col p-8 overflow-y-auto">
            <div className="flex justify-between items-end mb-8">
                <div>
                    <h1 className="text-3xl font-extrabold text-brown-900 mb-2 flex items-center gap-2">
                        <Palette
                    ...
                    <p className="text-brown-500 font-medium">
                        로컬 자산 라이브러리가 API와 실시간으로 연동되었습니다.
                        (Phase 4-C)
                    </p>
                </div>
                <div className="flex gap-2">
                    <button className="bg-brown-800 text-white font-bold py-3 px-6 rounded-2xl shadow-[4px_4px_0_0_#000] hover:-translate-y-1 hover:shadow-[6px_6px_0_0_#000] active:translate-y-1 active:shadow-none transition-all border-2 border-black">
                        + 신규 에셋 생성
                    </button>
                    {onClose && (
                        <button
                            onClick={onClose}
                            className="w-12 h-12 flex items-center justify-center bg-gray-200 text-gray-700 font-bold rounded-2xl shadow-[4px_4px_0_0_#000] hover:-translate-y-1 hover:shadow-[6px_6px_0_0_#000] active:translate-y-1 active:shadow-none transition-all border-2 border-black"
                            title="스튜디오 닫기"
                        >
                        ...
                        return (
                            ...
                            >
                                ...
                                >
                                    ...
                                    {isSelected ? (
                                        <span className="text-xs bg-black text-white px-2 py-1 rounded-md font-black shrink-0">
                                            선택됨
                                        </span>
                                    ) : (
                                        <span className="text-xs bg-pink-100 text-pink-600 px-2 py-1 rounded-full font-bold shrink-0">
                                            Image
                                        </span>
                                    )}
                                </div>
